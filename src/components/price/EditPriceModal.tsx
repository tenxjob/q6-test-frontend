...
import { useUpdatePrice, type PriceItem, type UpdatePriceInput } from '../../service/apiPrice';
...
export function EditPriceModal({ isOpen, priceItem, onClose }: EditPriceModalProps) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  useEffect(() => {
    if (priceItem) {
      reset({
        usd: priceItem.usd,
        roundedUsd: priceItem.roundedUsd ?? Math.round(priceItem.usd),
        price: priceItem.price,
        isUse: priceItem.isUse,
      });
    }
  }, [priceItem, reset]);
  ...
  const onSubmit = (data: UpdatePriceInput) => {
    ...
    updateMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success(`อัปเดตเรทราคา $${usd.toFixed(2)} (${price.toLocaleString()} ฿) สำเร็จ`);
          onClose();
        },
        onError: (err: any) => {
          toast.error(err?.message || 'เกิดข้อผิดพลาดในการอัปเดตเรทราคา');
        },
      }
    );
  };
